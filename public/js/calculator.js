...
class Calculator {
  ...
  initializeElements() {
    ...
    this.appTimeInput = document.getElementById('appTime');
    this.proHourlyInput = document.getElementById('proHourly');
    this.totalHoursInput = document.getElementById('totalHours');
    this.numProsInput = document.getElementById('numPros');
    this.totalAppointmentsInput = document.getElementById('totalAppointments');
    this.hourlyRateInput = document.getElementById('hourlyRate');
    this.earlyArrivalInput = document.getElementById('earlyArrival');
    this.retouchingCostInput = document.getElementById('retouchingCost');
    this.discountPercentInput = document.getElementById('discountPercent');
    this.eventsPerYearInput = document.getElementById('eventsPerYear');
  }
  
  initializeEventListeners() {
    eventManager.on(this.serviceTypeSelect, 'change', () => {
      this.updateServiceTypeFields();
      this.updateTotalAppointments();
    });
    
    eventManager.on(this.totalHoursInput, 'input', () => this.updateTotalAppointments());
    eventManager.on(this.appTimeInput, 'input', () => this.updateTotalAppointments());
    eventManager.on(this.numProsInput, 'input', () => this.updateTotalAppointments());
  }
  
  updateServiceTypeFields() {
    const serviceType = this.serviceTypeSelect.value;
    if (!Validation.validateServiceType(serviceType)) {
      ErrorHandler.showError('Invalid service type selected');
      return;
    }
    
    this.setDefaultValues(serviceType);
    this.toggleHeadshotFields(serviceType);
  }
  
  setDefaultValues(serviceType) {
    const serviceConfig = Settings.SERVICE_TYPES[serviceType];
    if (serviceConfig) {
      this.appTimeInput.value = serviceConfig.defaultAppTime;
      this.proHourlyInput.value = serviceConfig.defaultProHourly;
    }
  }
  
  toggleHeadshotFields(serviceType) {
    ...
    const isHeadshot = serviceType === 'headshot';
    
    headshotFields.forEach(field => field.style.display = isHeadshot ? 'block' : 'none');
    nonHeadshotFields.forEach(field => field.style.display = isHeadshot ? 'none' : 'block');
  }
  
  updateTotalAppointments() {
    const totalHours = parseFloat(this.totalHoursInput.value);
    const appTime = parseFloat(this.appTimeInput.value);
    const numPros = parseInt(this.numProsInput.value);
    
    if (!this.validateInputs(totalHours, appTime, numPros)) {
      return;
    }
    
    const totalAppts = this.calculateTotalAppointments(totalHours, appTime, numPros);
    this.totalAppointmentsInput.value = totalAppts;
  }
  
  validateInputs(totalHours, appTime, numPros) {
    const { MIN_HOURS, MAX_HOURS, MIN_APPOINTMENT_TIME, MAX_APPOINTMENT_TIME, MIN_PROFESSIONALS, MAX_PROFESSIONALS } = Settings.VALIDATION;
    
    if (!Validation.validateNumber(totalHours, MIN_HOURS, MAX_HOURS)) {
      ErrorHandler.showError(`Total hours must be between ${MIN_HOURS} and ${MAX_HOURS}`);
      return false;
    }
    
    if (!Validation.validateNumber(appTime, MIN_APPOINTMENT_TIME, MAX_APPOINTMENT_TIME)) {
      ErrorHandler.showError(`Appointment time must be between ${MIN_APPOINTMENT_TIME} and ${MAX_APPOINTMENT_TIME} minutes`);
      return false;
    }
    
    if (!Validation.validateNumber(numPros, MIN_PROFESSIONALS, MAX_PROFESSIONALS)) {
      ErrorHandler.showError(`Number of professionals must be between ${MIN_PROFESSIONALS} and ${MAX_PROFESSIONALS}`);
      return false;
    }
  ...
  calculateTotalAppointments(totalHours, appTime, numPros) {
    const apptsPerHourPerPro = 60 / appTime;
    const totalApptsPerHour = apptsPerHourPerPro * numPros;
    return Math.floor(totalHours * totalApptsPerHour);
  }
  ...
  calculate() {
    ...
    const appTime = parseFloat(this.appTimeInput.value);
    const numPros = parseInt(this.numProsInput.value);
    const proHourly = parseFloat(this.proHourlyInput.value);
    const discountPercent = parseFloat(this.discountPercentInput.value);
    const eventsPerYear = parseInt(this.eventsPerYearInput.value);
    ...
    const totalAppts = this.calculateTotalAppointments(totalHours, appTime, numPros);
    const { totalCost, totalProRev } = this.calculateCosts(serviceType, totalHours, numPros, proHourly, totalAppts);
    const { shortcutNet, shortcutMargin } = this.calculateShortcutMetrics(totalCost, totalProRev);
    const totalAnnual = this.calculateAnnualValue(totalCost, eventsPerYear);
    ...
    // Save draft
    storageManager.saveDraft(result);
    
    return result;
  }
  
  calculateCosts(serviceType, totalHours, numPros, proHourly, totalAppts) {
    let totalCost = 0;
    let totalProRev = 0;
    
    if (serviceType === 'headshot') {
      const retouchingCost = parseFloat(this.retouchingCostInput.value);
      totalProRev = totalHours * numPros * proHourly;
      totalCost = totalProRev + (totalAppts * retouchingCost);
    } else {
      const hourlyRate = parseFloat(this.hourlyRateInput.value);
      const earlyArrivalBase = parseFloat(this.earlyArrivalInput.value);
      const totalEarlyArrival = earlyArrivalBase * numPros;
      totalProRev = (totalHours * numPros * proHourly) + totalEarlyArrival;
      totalCost = (totalHours * hourlyRate * numPros);
    }
    
    return { totalCost, totalProRev };
  }
  
  calculateShortcutMetrics(totalCost, totalProRev) {
    const shortcutNet = totalCost - totalProRev;
    const shortcutMargin = (shortcutNet / totalCost) * 100;
    return { shortcutNet, shortcutMargin };
  }
  
  calculateAnnualValue(totalCost, eventsPerYear) {
    return totalCost * eventsPerYear;
  }
}
